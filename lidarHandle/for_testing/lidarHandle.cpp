...
#define EXPORTING_lidarHandle 1
    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("lidarHandle"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return lidarHandleInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "lidarHandle", nlhs, plhs, nrhs, prhs);
}

LIB_lidarHandle_CPP_API 
void MW_CALL_CONV lidarHandle(int nargout, mwArray& dataZtj, mwArray& dataDensity, const 
                              mwArray& filename)
{
    mclcppMlfFeval(_mcr_inst, "lidarHandle", nargout, 2, 1, &dataZtj, &dataDensity, &filename);
}
